...
bool AppendMail(const wchar_t *w1,const CkEmail &email);
...
bool CopyMultiple(CkMessageSet &messageSet,const wchar_t *w1);
...
bool FetchAttachment(CkEmail &email,int attachIndex,const wchar_t *w1);
bool FetchAttachmentBytes(CkEmail &email,int attachIndex,CkByteData &outBytes);
bool FetchAttachmentString(CkEmail &email,int attachIndex,const wchar_t *w1,CkString &outStr);
CkEmailBundleW *FetchBundle(const CkMessageSet &messageSet);
CkStringArrayW *FetchBundleAsMime(const CkMessageSet &messageSet);
CkEmailBundleW *FetchChunk(int startSeqNum,int count,CkMessageSet &failedSet,CkMessageSet &fetchedSet);
bool FetchFlags(long msgId,bool bUid,CkString &outStrFlags);
CkEmailBundleW *FetchHeaders(const CkMessageSet &messageSet);
...
long GetMailFlag(const CkEmail &email,const wchar_t *w1);
...
bool SetFlags(const CkMessageSet &messageSet,const wchar_t *w1,long value);
bool SetMailFlag(const CkEmail &email,const wchar_t *w1,long value);
...
bool SshAuthenticatePk(const wchar_t *w1,CkSshKey &privateKey);
...
const wchar_t *fetchAttachmentString(CkEmail &email,int attachIndex,const wchar_t *w1);
...
const wchar_t *getMailAttachFilename(const CkEmail &email,long attachIndex);
